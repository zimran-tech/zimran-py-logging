...
def test_sensitive_info_in_message(gdpr_patcher):
    ...
    assert record["message"] == (
        "User has been registered: user@zimran.io. - This log possibly contains non-compliant fields."
    )
...
def test_sensitive_info_in_extra(gdpr_patcher):
    ...
    assert record["message"] == "Regular message. - This log possibly contains non-compliant fields."
...
def test_sensitive_info_in_extra_complex_structure(gdpr_patcher):
    record = {
        ...
        "extra": {
            "user_data": {"ssn": "123-45-6789", "name": "John Doe"},
            "user_id": "1234"
        }
    }
    gdpr_patcher(record)
